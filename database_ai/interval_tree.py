...
t1 = time()
# === 1. 读取基因区间文件 ===
gene_df = pd.read_csv("tss_tran/NC_000024.10.tsv", sep="\t", header=None, names=["trans", "tss", "gene", "gene_ori", "gene_start", "gene_end", "gene_id", "gene_type"])
...
# === 2. 读取gRNA文件 ===
grna_df = pd.read_csv("nag_remove/NC_000024.10.tsv", sep="\t", header=None)
...
# 可选：保存结果
grna_df.to_csv("grna_annotated.tsv", sep="\t", index=False, header=None)
