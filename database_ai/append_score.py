...
def score_res2df(nc_no: str) -> pd.DataFrame:
    file_li = sorted(list(Path(f"/mnt/ntc_data/wayne/Repositories/CRISPR/database_ai/cfd_score/flashfry_filter_score_out/").glob(f"{nc_no}*.tsv")),key=lambda x: int(x.stem.split('_')[-1]))
    if len(file_li) == 1:
...
def gdb_reset_sample_id(gdb_df: pd.DataFrame) -> None:
    new_id = gdb_df[7].to_numpy() + ["_"] * len(gdb_df[0]) + gdb_df[0].astype(str).to_numpy()
    gdb_df[0] = new_id
...
def run_append(nc_no: str) -> None:
    ...
    append_path = f"/mnt/ntc_data/wayne/Repositories/CRISPR/database_ai/cfd_score/{nc_no}.tsv"
