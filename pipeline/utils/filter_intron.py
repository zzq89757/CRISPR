import pandas as pd
from .gene_annotate import scaffold_detective_numpy
...
def exon_annotate(project_dir: str,nc_no: str) -> None:
    # 读取gdb
    gdb_file = f"{project_dir}/gene_annotated/{nc_no}.tsv"
    gdb_df = gdb2df(gdb_file)
    # 读取对应的exon表
    exon_file = f"{project_dir}/GCF/gtf/{nc_no}/EXON.tsv"
    exon_df = region2df(exon_file)
    # 注释gdb
    output_file = f"{project_dir}/intron_filtered/{nc_no}.tsv"
...
def main() -> None:
    ...
    exon_annotate("/mnt_data/Wayne/Repositories/CRISPR/pipeline/mus/", nc_li[-1])
