...
def utr_region_obtain(exon_file_path: str, cds_file_path: str, gene_pos_file: str) -> defaultdict:
    gene_df: pd.DataFrame = gdb2df(gene_pos_file)
    ori_dict = dict(zip(gene_df[0], gene_df[3]))
    # 读取exon和cds region 文件
    exon_df = gdb2df(exon_file_path)
    cds_df = gdb2df(cds_file_path)  
...
def search_regions(grna_table: str, region_dict: defaultdict) -> pd.DataFrame:
    gdb_df = gdb2df(grna_table)
    gdb_df["Target Region"] = gdb_df.apply(lambda x:region_classify(x[8],x[7],x[4],region_dict),axis=1)
    return gdb_df   


# 根据UTR位置 添加新列 region 标记UTR和CDS
def utr_mark(project_dir: str, nc_no: str) -> None:
    gene_file = f"{project_dir}/GCF/gtf/{nc_no}/GENE.tsv"
    exon_file = f"{project_dir}/GCF/gtf/{nc_no}/EXON.tsv"
    cds_file = f"{project_dir}/GCF/gtf/{nc_no}/CDS.tsv"
    gdb_file = f"{project_dir}/snp_mark/{nc_no}.tsv"
    res_file = f"{project_dir}/utr_mark/{nc_no}.tsv"
    utr_pos_dict = utr_region_obtain(exon_file, cds_file, gene_file) 
