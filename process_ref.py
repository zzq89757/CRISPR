...
def async_in_iterable_structure(fun, fun_args, iterable_structure, cpus):
    def init(l):
        global lock
        lock = l
    ...
    for i in iterable_structure:
        p.apply_async(func=fun, args=[i, fun_args])
    p.close()
    p.join()
...
def obtain_ref_info(ref: str) -> list:
    ref_handle = FastaFile(ref)
    return ref_handle.references, ref_handle


def generate_sgRNA_table(chr_name: str, ref):
...
def main() -> None:
    chr_li, ref_handle = obtain_ref_info("/mnt/ntc_data/wayne/DataBase/Homo/hg38.fa")
    async_in_iterable_structure(generate_sgRNA_table, ref_handle[:2], chr_li, 24)